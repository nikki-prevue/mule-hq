...
export async function getOffices() {
  ...
  return res.results.map(p => ({
    id: p.id,
    name: p.properties['Office']?.title[0]?.plain_text || '',
    doctor: p.properties['Doctor(s)']?.rich_text[0]?.plain_text || '',
    city: p.properties['City']?.select?.name || '',
    tier: p.properties['Referral Tier']?.select?.name || 'Moderate',
    address: p.properties['Address']?.rich_text[0]?.plain_text || '',
    contact: p.properties['Contact']?.rich_text[0]?.plain_text || '',
    notes: p.properties['Notes']?.rich_text[0]?.plain_text || '',
    gift: p.properties['Gift/Materials']?.rich_text[0]?.plain_text || '',
    lastVisit: p.properties['Last Visited']?.date?.start || null,
    nextAction: p.properties['Follow-ups']?.rich_text[0]?.plain_text || '',
    status: p.properties['Status']?.select?.name || '',
    referralVolume: p.properties['Referral Volume 12mo']?.number || 0,
    zip: p.properties['Zip']?.rich_text[0]?.plain_text || '',
  }));
}
...
export async function updateOffice(id, data) {
  const props = {};
  if (data.tier) props['Referral Tier'] = { select: { name: data.tier } };
...
export async function getSupplies() {
  const res = await notion.databases.query({ database_id: DB.supplies, page_size: 100 });
  return res.results.map(p => ({
    id: p.id,
