          ...
          <div style={modalBox}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
              <div style={{flex:1,paddingRight:12}}>
                <div style={{fontSize:20,fontWeight:600,color:C.choc,marginBottom:3}}>{ao.name}</div>
                <div style={{fontSize:13,color:C.choc2}}>{docs.length?docs.map(d=>d.name).join(', '):(ao.doctor||'No doctor on file')}</div>
                <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:8,alignItems:'center'}}>
                  <span style={badge(ao.tier==='hot'?C.hot:C.gold)}>{ao.tier}</span>
                  {ao.topReferrer&&<span style={badge(C.gold)}>Top referrer</span>}
            ...
            <div style={{background:C.glassDark,borderRadius:12,padding:'10px 14px',marginBottom:12,fontSize:13,color:C.choc2,lineHeight:1.9}}>
              {ao.address&&<div><span style={{color:C.choc3}}>Address  </span><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(ao.address+' '+(ao.city||'')+' TX')}`} target="_blank" rel="noreferrer" style={{color:C.gold,textDecoration:'none'}}>{ao.address}{ao.city?', '+ao.city:''}</a></div>}
              {ao.phone&&<div><span style={{color:C.choc3}}>Phone  </span><a href={`tel:${String(ao.phone).replace(/[^0-9]/g,'')}`} style={{color:C.gold,textDecoration:'none'}}>{ao.phone}</a></div>}
              {ao.hours&&<div><span style={{color:C.choc3}}>Hours  </span>{ao.hours}</div>}
              {ao.closedDays&&<div><span style={{color:C.choc3}}>Closed  </span><span style={{color:C.hot}}>{ao.closedDays}</span></div>}
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              ...
              {ao.address&&<button style={btn.secondary} onClick={()=>window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(ao.address+' '+(ao.city||'')+' TX')}`,'_blank')}>Maps</button>}
              <button style={btn.secondary} onClick={()=>setShowEdit(x=>!x)}>{showEdit?'Hide details':'Edit details'}</button>
            </div>
